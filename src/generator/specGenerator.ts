...
export class SpecGenerator {
  ...
  private buildDefinitions() {
    let definitions: { [definitionsName: string]: Swagger.Schema } = {}
    Object.keys(this.metadata.referenceTypeMap).map(typeName => {
      const referenceType = this.metadata.referenceTypeMap[typeName]
      const newType = this.buildDefinition(referenceType)
      definitions = {
        ...definitions,
        ...newType
      }
  ...
  private buildDefinition(referenceType) {
    let definitions: { [definitionsName: string]: Swagger.Schema } = {}
    // Object definition
    if (referenceType.properties) {
      ...
      // for (var p in referenceType.properties) {
      //   if (
      //     referenceType.properties[p].type &&
      //     referenceType.properties[p].type.dataType === 'object'
      //   ) {
      //     const propertyType = this.buildDefinition(referenceType.properties[p].type)
      //     definitions = {
      //       ...definitions,
      //       ...(propertyType || {})
      //     }
  ...
  private buildMethod(
    ...
    pathMethod.tags = method.tags || [controllerName.replace('Controller', '')]
    ...
    if (
      pathMethod.parameters.filter(
        (p: Swagger.BaseParameter) => p && p.in === 'body'
      ).length > 1
    ) {
