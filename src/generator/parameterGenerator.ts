...
export class ParameterGenerator {
  ...
  public Generate(): Tsoa.Parameter {
    let decoratorName = getDecoratorName(this.parameter, identifier =>
      this.supportParameterDecorator(identifier.text)
    )

    // handle default parameter types based on method type
    if (!decoratorName) {
      switch (this.method) {
        case 'get':
        case 'delete':
    ...
    switch (decoratorName) {
      ...
      case 'Response':
        return this.getResponseParameter(this.parameter)
      case 'Body':
