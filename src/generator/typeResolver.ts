...
export class TypeResolver {
  ...
  public resolve(): Tsoa.Type {
    ...
    if (this.typeNode.kind === ts.SyntaxKind.TypeLiteral) {
      var memberTypes: any[] = []
      if ((this.typeNode as ts.TypeLiteralNode).members) {
        for (var memberTypeNode of (this.typeNode as ts.TypeLiteralNode)
          .members) {
          var memberType = new TypeResolver(
            // @ts-ignore
            memberTypeNode.type,
            this.current
          ).resolve()
    ...
    if (typeReference.typeName.kind === ts.SyntaxKind.Identifier) {
      ...
      if (typeReference.typeName.text === 'Object') {
        return new TypeResolver(
          typeReference.typeArguments[0],
          this.current
        ).resolve()
